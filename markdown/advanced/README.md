...
![An actual CPU load graph from the BeepBeep 3 system evaluating a query on a large event trace. The left part of the graph shows the query in single-threaded mode; the right part shows the same query running with multi-threading enabled.](cpu-load-screenshot.png)
...
![Sequence diagram where no preemptive occurs.](ppull-sequential.png)
...
![Sequence diagram where W performs preemptive pulling on *P* in a separate thread.](ppull-sequential.png)
