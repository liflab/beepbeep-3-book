...
{@img doc-files/basic/PipingUnaryMissing.png}{Forgetting to pipe two processors.}{.6}
...
{@snipm basic/PipingUnaryIncorrect.java}{/}
...
{@snipm basic/PipingBinaryWait.java}{/}
...
{@img doc-files/basic/PipingBinaryWait.png}{A fast source and a slow source.}{.6}
...
A processor can be queried for the types it accepts for input number *n* by using the method {@link jdm:ca.uqac.lif.cep.Processor#getInputType(int) getInputType()}; likewise for the type produced at output number *n* with {@link jdm:ca.uqac.lif.cep.Processor#getOutputType(int) getOutputType()}.
...
{@snipm basic/PipingUnaryPush.java}{/}

{@img doc-files/basic/PipingUnaryPush.png}{Pushing events to a `Print` processor.}{.6}
