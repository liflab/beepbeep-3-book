...
- It is not a **high-performance computing environment**. Many things can be done reasonably well in BeepBeep, and there are numerous use cases for which it is more than fast enough. However, if you expect to crunch data at speeds of exabytes per second, chances are BeepBeep will be too slow for your task.
